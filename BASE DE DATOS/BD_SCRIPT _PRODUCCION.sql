...
CREATE OR ALTER PROCEDURE [dbo].[ObtenerNominaMensualEmpleado]
@EMPLEADO_ID BIGINT
AS 
BEGIN
    DECLARE @MES INT
    ...
    SELECT 
        1 AS ORDEN,
        'INGRESO' AS NOMBRE_TIPO,
		ingreso.ID_INGRESONOMINADETALLE AS CONSECUTIVO,
        ingre.NOMBRE_INGRESO AS TIPO, 
        ingreso.DETALLE, 
        ingreso.MONTO, 
    ...
    SELECT  
        2 AS ORDEN,
        'DEDUCCION' AS NOMBRE_TIPO,
		DEDU.ID_DEDUCCIONNOMINADETALLE AS CONSECUTIVO,
        DEDUCCION.NOMBRE_DEDUCCION AS TIPO, 
        DEDU.DETALLE, 
        DEDU.MONTO,  
        0 AS CANTIDAD 
    FROM 
...
select nomina.DESCRIPCION AS NOMINA , NOMINA.FECHA_PAGO, emple.NOMBRECOMPLETO as EMPLEADO , emple.ID_EMPLEADO , emple.IDENTIFICACION , cargo.DESCRIPCION as CARGO, nomina.SALARIO_NETO  as SALARIO, nomina.ESTADO from NOMINA nomina
...
BEGIN
	...
	SELECT	IND.[MONTO],[DETALLE], IND.ID_INGRESONOMINADETALLE AS CONSECUTIVO, IND.[INGRESO_ID] AS TIPO_ID , ING.NOMBRE_INGRESO AS NOMBRE_TIPO, E.NOMBRECOMPLETO AS NOMBRE_EMPLEADO,[CANTIDAD]
	FROM [dbo].[INGRESONOMINADETALLE] IND
	INNER JOIN EMPLEADO E ON IND.EMPLEADO_ID = E.ID_EMPLEADO
	INNER JOIN INGRESO ING ON IND.INGRESO_ID = ING.ID_INGRESO
	WHERE ID_INGRESONOMINADETALLE = @INGRESO_ID

END
GO

CREATE OR ALTER   PROCEDURE [dbo].[ObtenerDeduccionDetalle]
@DEDUCCION_ID BIGINT
AS
BEGIN

	SELECT	DND.[MONTO],[DETALLE], DND.ID_DEDUCCIONNOMINADETALLE AS CONSECUTIVO, [DEDUCCION_ID] AS TIPO_ID, DEDU.NOMBRE_DEDUCCION AS NOMBRE_TIPO, E.NOMBRECOMPLETO AS NOMBRE_EMPLEADO
	FROM [dbo].[DEDUCCIONNOMINADETALLE] DND
	INNER JOIN EMPLEADO E ON DND.EMPLEADO_ID = E.ID_EMPLEADO
	INNER JOIN DEDUCCION DEDU ON DND.DEDUCCION_ID = DEDU.ID_DEDUCCION
	WHERE  ID_DEDUCCIONNOMINADETALLE = @DEDUCCION_ID

END
GO

create OR ALTER TRIGGER [dbo].[trg_AfterInsert_AspNetUsers]
ON [dbo].[AspNetUsers]
AFTER INSERT
AS
BEGIN
    BEGIN TRY
        ...
        SET @ImagenPorDefecto = 0x4E; -- Imagen PNG 1x1 pixel

        -- Insertar en la tabla EMPLEADO con el CORREO_ID correspondiente al correo.
        INSERT INTO EMPLEADO (FECHA_INGRESO, FOTO, SALARIO,SALDO_VACACIONES, CORREO_ID, HORARIOLABORAL_ID, ESTADO, AspNetUsers_ID)
        SELECT GETDATE(), @ImagenPorDefecto, 0.00, 083, C.ID_CORREO, 1, 1, ASP.Id
        FROM CORREO C
        INNER JOIN INSERTED I ON C.CORREO = I.Email
